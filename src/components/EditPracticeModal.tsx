...
import { PracticeSection } from '@/services/chapterService';
...
export default function EditPracticeModal({ isOpen, onClose, practice, onSubmit }: EditPracticeModalProps) {
  ...
  useEffect(() => {
    if (practice) {
      ...
      setAnswer(practice.answers && practice.answers.length > 0 ? practice.answers[0] : '');
      setAnswerType(practice.answerType || 'text');
      setImageFile(null); // Reset file input for edit
      setAnswerImageFile(null); // Reset answer image file input for edit
    }
  }, [practice]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!title.trim() && !description.trim() && !imageFile && !practice.imageUrl) {
    ...
    if (answerType === 'text' || answerType === 'code') {
      ...
      if (!answerImageFile && !practice.answerImageUrl) {
        alert('Please select an answer image or keep the existing one');
        return;
      }
    } else if (answerType === 'mixed') {
      if (!answer.trim() && !answerImageFile && !practice.answerImageUrl) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 w-full max-w-2xl mx-4 max-h-screen overflow-y-auto">
        <h2 className="text-xl font-semibold text-gray-900 mb-4">Edit Practice Section</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">
              {practice.imageUrl ? 'Upload new image to replace current one' : 'Upload an image file (JPG, PNG, GIF) - Optional'}
            </p>
          ...
          {(answerType === 'image' || answerType === 'mixed') && (
            <div>
              ...
              {practice.answerImageUrl && (
                <div className="mb-2 p-2 bg-white rounded border">
                  <p className="text-xs text-gray-600 mb-1">Current Answer Image:</p>
                  <img 
                    src={practice.answerImageUrl} 
              ...
              />
              <p className="text-xs text-gray-500 mt-1">
                {practice.answerImageUrl ? 'Upload new image to replace current answer image' : 'Upload an image file for the answer (JPG, PNG, GIF)'}
              </p>
            </div>
          )}
